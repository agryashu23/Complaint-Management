...
class CompletedComplaintsContractFragment extends StatefulWidget {
  const CompletedComplaintsContractFragment({super.key});

  @override
  State<CompletedComplaintsContractFragment> createState() => _CompletedComplaintsContractFragmentState();
}

class _CompletedComplaintsContractFragmentState extends State<CompletedComplaintsContractFragment> {
  ...
  void fetchComplaints() async {
    ...
    print('HERE');
    final response = await http.get(Uri.parse('http://143.110.177.156:3000/mobile/contracterlist-complaints/${userID}'));
    if (response.statusCode == 200) {
      // final List<dynamic> jsonResponse = json.decode(response.body);
      final List<dynamic> jsonResponse = json.decode(response.body);
      print(jsonResponse);
      final pendingComplaints = jsonResponse.where((item) => (item['status'] == 'resolved'||item['status'] == 'verified'||item['status'] == 'verification'));
      setState(() {
