...
class PendingComplaintsFragment extends StatefulWidget {
  const PendingComplaintsFragment({super.key});

  @override
  State<PendingComplaintsFragment> createState() => _PendingComplaintsFragmentState();
}

class _PendingComplaintsFragmentState extends State<PendingComplaintsFragment> {
  ...
  void fetchComplaints() async {
    ...
    final response = await http.get(Uri.parse('http://143.110.177.156:3000/mobile/complaints/${userID}'));
    if (response.statusCode == 200) {
      // final List<dynamic> jsonResponse = json.decode(response.body);
      final List<dynamic> jsonResponse = json.decode(response.body);
      final pendingComplaints = jsonResponse.where((item) => item['status'] != 'resolved');
      setState(() {
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return ListTile(
            ...
            title: Container(
              ...
              child: Text(splitString(complaint.id),style: GoogleFonts.aBeeZee(
                color: Colors.black,
