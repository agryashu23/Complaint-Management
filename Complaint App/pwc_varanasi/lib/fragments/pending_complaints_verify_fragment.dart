...
class PendingComplaintsVerifyFragment extends StatefulWidget {
  const PendingComplaintsVerifyFragment({super.key});

  @override
  State<PendingComplaintsVerifyFragment> createState() => _PendingComplaintsVerifyFragmentState();
}

class _PendingComplaintsVerifyFragmentState extends State<PendingComplaintsVerifyFragment> {
  ...
  void fetchComplaints() async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<dynamic> jsonResponse = json.decode(response.body);
      final pendingComplaints = jsonResponse.where((item) => item['status'] == 'verification');
      setState(() {
      ...
      body: isDataFetched?Container(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return ListTile(
            ...
            onTap: () async {
              await Navigator.push(context, MaterialPageRoute(builder: (context)=>ComplaintDetailScreen(complaint)));
              fetchComplaints();
            },
          );
        },
