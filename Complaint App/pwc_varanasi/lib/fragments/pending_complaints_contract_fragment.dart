...
import 'package:pwc_varanasi/screens/complain_detail_contract_screen.dart';
import 'package:pwc_varanasi/screens/complain_detail_screen.dart';
...
class PendingComplaintsContractFragment extends StatefulWidget {
  const PendingComplaintsContractFragment({super.key});

  @override
  State<PendingComplaintsContractFragment> createState() => _PendingComplaintsContractFragmentState();
}

class _PendingComplaintsContractFragmentState extends State<PendingComplaintsContractFragment> {
  ...
  void fetchComplaints() async {
    ...
    final response = await http.get(Uri.parse('http://143.110.177.156:3000/mobile/contract-complaints/${userID}'));
    if (response.statusCode == 200) {
      // final List<dynamic> jsonResponse = json.decode(response.body);
      final List<dynamic> jsonResponse = json.decode(response.body);
      final pendingComplaints = jsonResponse.where((item) => item['status'] == 'inProgress');
      setState(() {
      ...
      body: isDataFetched?Container(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return ListTile(
            ...
            onTap: () async {
              await Navigator.push(context, MaterialPageRoute(builder: (context)=>ComplaintDetailContractScreen(complaint)));
              fetchComplaints();
            },
          );
        },
